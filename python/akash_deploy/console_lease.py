...
@dataclass
class GetLeaseStatusResponseForwardedPortList:
    """Wrapper messages for map\<string, repeated T> (proto3 limitation)"""
    items: List[GetLeaseStatusResponseForwardedPort] = field(default_factory=list)
    TYPE_URL: str = field(default="/console.lease.GetLeaseStatusResponseForwarded_portList", init=False, repr=False)
    ...
    @classmethod
    def decode(cls, data: bytes) -> "GetLeaseStatusResponseForwardedPortList":
...
@dataclass
class GetLeaseStatusResponseIpList:
    items: List[GetLeaseStatusResponseIp] = field(default_factory=list)
    TYPE_URL: str = field(default="/console.lease.GetLeaseStatusResponseIpList", init=False, repr=False)
    ...
    @classmethod
    def decode(cls, data: bytes) -> "GetLeaseStatusResponseIpList":
        """Decode from protobuf binary (requires native extension)."""
        from akash_deploy._native import decode_message  # type: ignore[import]
        return cls(**json.loads(decode_message(cls.TYPE_URL, data)))
